...
class iCaRLmodel:
    ...
    def __init__(self, numclass, feature_extractor, img_size, batch_size, task_size, memory_size, epochs,
                 learning_rate):
    ...
    def _get_old_model_output(self, dataloader):
        ...
        for step, (indexs, imgs, labels) in enumerate(dataloader):
            ...
            with torch.no_grad():
                old_model_output = torch.sigmoid(self.model(imgs))
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for step, (indexs, images, target) in enumerate(self.train_loader):
                ...
                loss_value = self._compute_loss(indexs, output, target)
                opt.zero_grad()
                loss_value.backward()
                opt.step()
    ...
    def _compute_loss(self, indexs, output, target):
        target = get_one_hot(target, self.numclass)
        output, target = output.to(device), target.to(device)
        if self.old_model_output == None:
    ...
    def afterTrain(self):
        ...
        self.model.train()
        return self._test(self.test_loader, 0)
